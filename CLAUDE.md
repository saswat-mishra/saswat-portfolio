...
- **Per-route head:** every route renders `<Seo>` (`src/seo/Seo.jsx`, react-helmet-async) → title/meta/canonical/OG/Twitter + JSON-LD (Person/Service/Article/FAQPage/BreadcrumbList via `src/seo/jsonld.js`). **react-helmet-async does NOT populate its SSR context under React 19 + `renderToPipeableStream`** — it renders its tags inline in the streamed body instead. So `entry-server.jsx` `liftHead()` extracts every head-type tag (`<title>/<meta>/<link>/<script ld+json>`) out of the rendered markup (the app itself emits none), marks each `data-rh="true"`, and returns them as `head`; the prerenderer injects them into `index.html`'s `<!--app-head-->` (clean `<head>`), and the body goes into `<!--app-html-->` inside `#root`. On the client, `main.jsx` removes the prerendered `[data-rh]:not(script)` tags before hydration so Helmet re-applies exactly one `<title>/<meta>/<link>` set (no duplicates); the lifted JSON-LD `<script>`s are kept (Helmet doesn't re-emit scripts on React 19). Net: clean `<head>` for both no-JS crawlers and hydrated clients.
...
- **Markdown blog:** articles can be authored as `src/content/blog/posts/*.md` (frontmatter + body). `plugins/vite-markdown.js` compiles each `.md` → a data module **at build time** (so `marked` runs in Node and never ships to the client): it extracts the TL;DR blockquote (`tldrHtml`), the body (`bodyHtml`, with `id`s baked onto `<h2>/<h3>`), a `toc`, the `## Frequently asked questions` Q&A (`faq[]` → accordion + FAQPage JSON-LD), and the `<!-- Internal links: … -->` comment (`related.{services,caseStudies,articles}`). `manifest.js` globs both `./blog/*.js` and `./blog/posts/*.md` into `blog`. `BlogPost.jsx` is dual-path: markdown posts render `bodyHtml` via `dangerouslySetInnerHTML` (styled by `.article-body` in `index.css` — cyberpunk but high-contrast), legacy JS posts render typed blocks via `RichBody`. `rss.xml` is generated in `prerender.mjs` from `blog` (discoverable via `<link rel=alternate>` in `index.html`).
...
## Entity, schema & technical SEO (implemented)
- **Canonical bio (entity consistency).** ONE bio string, defined as `SITE.canonicalBio` in `src/site.config.js` and used **verbatim** in exactly three surfaces — the About page bio paragraph, the footer brand blurb, and the `description` of both Person and Organization JSON-LD. Do not fork it; search + AI answer engines cross-check the entity across these surfaces. The string:
...
- **Global JSON-LD graph (one canonical `@id` each).** `src/seo/jsonld.js`. Home emits `Person` (`#person`), `Organization` (`#org`, founder→`#person`), and `WebSite` (`#website`, publisher→`#org`) with a valid `SearchAction` → `/search?q={search_term_string}`. About emits `Person` + `Organization` + `ProfilePage`. Per-page schema (Service / BlogPosting / FAQPage / BreadcrumbList) lives on its route; `articleJsonLd` author/publisher are self-describing (name+url+`@id`) so Rich Results validates each page standalone. `sameAs` comes from `sameAs()` (non-empty `SITE.social` only: LinkedIn, GitHub, Upwork, X, Hugging Face; Crunchbase/Clutch are empty placeholders, omitted until set).
- **Search route.** `src/routes/Search.jsx` (`/search`) does client-side filtering over the content manifest; it is prerendered, `noindex`, and **excluded from sitemap** (`NOINDEX_ROUTES` in `manifest.js`) — its only SEO job is to make the WebSite `SearchAction` target real.
- **Meta hygiene.** Every route's `<Seo>` ships a unique `<title>`, meta description (answer-style, ≤~155), canonical, and per-page OG/Twitter image.
- **Per-page OG images.** `node scripts/generate-og-images.mjs` (runs first in `npm run build`) reads the content manifest and renders a branded 1200×630 PNG per route/article → `public/og-image.png` (default) + `public/og/<slug>.png` (services, work, blog, index pages). `public/og/` is gitignored (rebuilt each build). Routes pass `image="/og/<slug>.png"` to `<Seo>`.
- **Verification + analytics (config-driven, conditional).** `headExtras()` in `site.config.js` emits Google Search Console + Bing verification `<meta>`, the Cloudflare Web Analytics beacon, optional Plausible, and the Microsoft Clarity snippet — only for non-empty `SITE.verification` / `SITE.analytics` / `SITE.clarity` values. `entry-server` re-exports it and `prerender.mjs` injects it into **every** page's `<head>`. Empty tokens → nothing emitted (build stays clean); paste a token + rebuild → it appears site-wide.
- **Crawl.** `public/robots.txt` (allow search + AI engines incl. GPTBot/OAI-SearchBot/ClaudeBot/PerplexityBot/Google-Extended/Applebot-Extended/CCBot; links sitemap), `public/humans.txt`, `sitemap.xml` (per-page `lastmod` from content dates, noindex routes excluded), `rss.xml`, `llms.txt`.
- **CI performance budget.** `scripts/check-budget.mjs` (last step of `npm run build`) fails the build if any route's gzipped JS (shared shell ∪ route chunk) exceeds its budget (Home 260 KB, others 200 KB). This also structurally enforces the code-splitting law — a Three.js leak into a non-Home route would blow the budget.
